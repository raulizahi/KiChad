...
#ifdef __WXMSW__
// <winsock2.h> arrives through the wxWidgets MSW headers and declares a global
// `typedef unsigned int GROUP`, which the using-directive above renders ambiguous.  Declaring
// the name here hides the Windows typedef for every unqualified use in this namespace.
using GROUP = KICHAD::BLOCK_DIAGRAM::GROUP;
#endif
                        ...
                        const char* aFormat, std::string& aError )
{
    ...
    // Both arms are wxString so MSVC does not have to choose between wxString and wchar_t*.
    plotter.SetTitle( aTitle.IsEmpty() ? wxString( wxS( "Block diagram" ) ) : aTitle );
