				   ...
				   unsigned char* dst, int w, int h, int stride)
{
	...
	for (shape = image->shapes; shape != NULL; shape = shape->next) {
        ...
        /* KiChad: the vendored parser predates paint-order support, so paint fill then stroke. */
        for (j = 0; j < 2; j++) {
            paintOrder = j + 1;

            if (paintOrder == 1 && shape->fill.type != NSVG_PAINT_NONE) {
                nsvg__resetPool(r);
                r->freelist = NULL;
                r->nedges = 0;
            ...
            if (paintOrder == 2 && shape->stroke.type != NSVG_PAINT_NONE && (shape->strokeWidth * scale) > 0.01f) {
                nsvg__resetPool(r);
                r->freelist = NULL;
                r->nedges = 0;
